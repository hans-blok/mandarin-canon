...
REM   - VS Code tasks: via scripts\fetch_tasks.py
REM
REM ==============================================================================
...
REM Run agents fetch script with all arguments, log to file and console
python scripts\fetch_mandarin_agents.py %* > %logfile% 2>&1
set exit_code=%ERRORLEVEL%

REM If agents fetch succeeded, also fetch VS Code tasks for same value_stream_fase
if %exit_code% equ 0 (
    echo.>> %logfile%
    echo [WRAPPER] Start fetch_tasks.py voor %1>> %logfile%
    python scripts\fetch_tasks.py %1 >> %logfile% 2>&1
    set exit_code=%ERRORLEVEL%
)
